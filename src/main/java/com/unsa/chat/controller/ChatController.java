...
public class ChatController {
    ...
    @SendTo("/rooms/{roomId}")   //구독하고 있는 장소로 메시지 전송 (목적지)  -> WebSocketConfig Broker 에서 적용한건 앞에 붙어줘야됨
    public ChatMessage chat(@DestinationVariable Long roomId, ChatMessage message) {
        return chatService.saveChatMessage(roomId, message.getSender(), message.getMessage());
    }
